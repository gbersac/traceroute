...
int main(int argc, char **argv)
{
    ...
    for (sent = 0 ; sent < NB_PACKET ; ++sent)
    {
        ...
        ++sent;
        printf("%d packets sent\r", sent + 1);
        fflush(stdout);

        int bytes = sendto(sockfd, packet, sizeof(packet), 0, (sockaddr *)&pingaddr, sizeof(sockaddr_in));

        bytes = recvmsg(sockfd, inbuf	, sizeof(inbuf), 0, (sockaddr *)&pingaddr, (socklen_t *)&addrlen);
